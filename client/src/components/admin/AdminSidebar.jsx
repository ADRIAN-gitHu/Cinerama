...
const AdminSidebar = () => {
  ...
  const adminNavLinks =[
    ...
    { name:'Añadir Películas', path: '/admin/add-shows',icon: PlusSquareIcon},
    { name:'Listar Películas', path: '/admin/list-shows',icon: ListIcon},
    { name:'Listsr Reservas', path: '/admin/list-bookings',icon: ListCollapseIcon},
  ]
  return (
          ...
          {adminNavLinks.map((link,index)=>(
              <NavLink key={index} to={link.path} end className={({isActive})=>`relative flex items-center max-md:justify-center gap-2 w-full py-2.5 min-md:pl-10 first:mt-6 text-gray-400 ${isActive && 'bg-primary/15 text-primary group'}`}>
                {({isActive})=>(
                  ...
                  <span className={`w-1.5 h-10 rounded-l right-0 abolute ${isActive && 'bg-primary'}`}/>
                  </>
                )}
              </NavLink>
          ))}
