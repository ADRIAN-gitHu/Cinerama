...
const SeatLayout = () => {
  ...
  const handleSeatClick = (seatId) =>{
    if (!selectedTime) {
      return toast("Seleccionar el horario por favor.")
    }
    if(!selectedSeats.includes(seatId) && selectedSeats.length > 4){
      return toast("Solo se pueden seleccionar hasta 5 asientos")
    }
  ...
  return show ? (
    <div className='flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-30 md:pt-50'>
      {/* Available Timings */}
      <div className='w-60 bg-primary/10 boder border-primary/20 rounded-lg py-10 h-max md:sticky md:top-30'>
        <p className='text-lg font-semibold px-6'>Horarios Disponibles</p>
      ...
      <div className='relative flex-1 flex flex-col items-center max-md:mt-16'>
        ...
        <h1 className='text-2xl font-semibold mb-4'> Seleccione su asiento</h1>
        <img src={assets.screenImage} alt="screen"/>
        <p className='text-gray-400 text-sm mb-6'>PANTALLA</p>
